...
Original file is located at
    https://colab.research.google.com/drive/17RHQqTh1o0D3od0nGLzBOKRLT-TO6YPu
"""
...
DATA_DIR = '/content/drive/MyDrive/Dolin_Segmentation/dataset320'
height=320
width=320
...
class Dataset:
    ...
    CLASSES = ["background",'dolins',"deneme"]
    ...
    def __getitem__(self, i):
        ...
        mask = np.where(mask<127,0,1)#bu ra benim 
...
dataset = Dataset(x_train_dir, y_train_dir, classes=["background",'dolins'])
print(len(dataset))
...
dataset = Dataset(x_train_dir, y_train_dir, classes=['dolins'], augmentation=get_training_augmentation())
print(len(dataset))
...
check=0
save_path=os.path.join("/content/drive/MyDrive/Dolin_Segmentation/results",BACKBONE)
...
for i in range(cc):
  ...
  metrics_deneme=[sm.metrics.FScore(threshold=0.5)]
...
#--------------------save as txt------------
  ...
  f.write("Tarih zaman= "+str(datetime.datetime.now())+"\n"
    "Model= "+BACKBONE+"\n"
        ...
        "Validation iou (Mean)= "+str(scores[1])+"\n"
        "Validation F1 (Mean)= "+str(scores[2])+"\n"
        "Train iou (Mean)= "+str(history.history['iou_score'][-1])+"\n"
        "Train F1 (Mean)= "+str(history.history['f1-score'][-1])+"\n"
...
#History tekrar çağırmak için
hstry = pickle.load(open("/content/drive/MyDrive/Dolin_Segmentation/hst", "rb"))
plt.plot(hstry["val_loss"])
plt.show()
...
#mean metrics of validation data
scores = model.evaluate_generator(valid_dataloader)
...
for i in ids:
    ...
    cv2.polylines(rslts,contours[0],True,(0,255,0),3)
